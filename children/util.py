...
#    
    ...
    # record whether driver is at least 21 and is carrying a passenger who is less than 15
    tmp_person = df_person[df_person.index.droplevel(['per_no']).isin(analytic_sample.index)]
    analytic_sample['driving_with_children'] = tmp_person['age_lt15'].groupby(['year','st_case','veh_no']).any() & (get_driver(tmp_person)['age']<=21)
    
    # code driver types as types 1 & 2 in the two-type case, or 1, 2, 3, & 4 in the four-type case
    
    if mireps==False:
        analytic_sample['driver_type'] = numpy.nan
        analytic_sample.loc[(analytic_sample['drink_status']==0) & analytic_sample['driving_with_children'],'driver_type'] = 1
        analytic_sample.loc[(analytic_sample['drink_status']==0) & ~analytic_sample['driving_with_children'],'driver_type'] = 2
        analytic_sample.loc[(analytic_sample['drink_status']==1) & analytic_sample['driving_with_children'],'driver_type'] = 3
        analytic_sample.loc[(analytic_sample['drink_status']==1) & ~analytic_sample['driving_with_children'],'driver_type'] = 4
    else:
        for mirep in range(0,mireps):           
            analytic_sample['driver_type'+str(mirep+1)] = numpy.nan
            analytic_sample.loc[(analytic_sample['drink_status'+str(mirep+1)]==0) & analytic_sample['driving_with_children'],'driver_type'+str(mirep+1)] = 1
            analytic_sample.loc[(analytic_sample['drink_status'+str(mirep+1)]==0) & ~analytic_sample['driving_with_children'],'driver_type'+str(mirep+1)] = 2
            analytic_sample.loc[(analytic_sample['drink_status'+str(mirep+1)]==1) & analytic_sample['driving_with_children'],'driver_type'+str(mirep+1)] = 3
            analytic_sample.loc[(analytic_sample['drink_status'+str(mirep+1)]==1) & ~analytic_sample['driving_with_children'],'driver_type'+str(mirep+1)] = 4
